 ...
 *
 */
class Remove7DaysOutdatedSalesJob implements ShouldQueue
{
    use Dispatchable;
    ...
    public function handle()
    {
        DB::statement("
            ...
            SET quantity_sold_last_7_days = 0,
                updated_at = now()
            WHERE last_sold_at < DATE_SUB(now(), INTERVAL 7 DAY)
            AND IFNULL(quantity_sold_last_7_days, 0) != 0
        ");

        DB::unprepared("
            DROP TEMPORARY TABLE IF EXISTS temp_itemMovementsStatistics_3982179371312321313;

            CREATE TEMPORARY TABLE temp_itemMovementsStatistics_3982179371312321313 AS (
                SELECT inventory_id,
