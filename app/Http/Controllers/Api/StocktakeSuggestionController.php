...
class StocktakeSuggestionController extends Controller
{
    public function index(Request $request): AnonymousResourceCollection
    {
        $query = QueryBuilder::for(StocktakeSuggestion::class)
            ->allowedFilters(['sku', 'name', 'quantity', 'quantity_to_stocktake', 'warehouse.id', 'warehouse.code'])
            ->allowedSorts(['points', 'inventory.warehouse_id', 'inventory.shelf_location', 'inventory.quantity'])
