        ...
        CREATE OR REPLACE VIEW modules_rmsapi_products_quantity_comparison_view AS
            SELECT
             modules_rmsapi_products_imports.id  as record_id,
              ...
              (SELECT max(id) FROM  inventory_movements WHERE inventory_movements.inventory_id = inventory.id
             AND inventory_movements.description = "stocktake"
             AND inventory_movements.user_id = 1
            AND inventory_movements.created_at > data_sub(now(), interval 7 day)) as movement_id
            ...
            WHERE modules_rmsapi_products_imports.id IN (SELECT MAX(ID) FROM modules_rmsapi_products_imports GROUP BY warehouse_id, product_id);
        ');
    }
};
